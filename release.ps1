...
A desktop writing assistant with local AI-powered spelling and grammar correction.

**Installation:**
1. Download and extract **stet_portable.zip**.
2. Run **download_model.bat** to fetch the AI model (~1.8 GB).
3. Run **Stet.exe** (or run.bat).

**Requirements:**
- Windows 10/11 (64-bit)
- NVIDIA GPU recommended for GPU-accelerated AI inference
- ~1.8 GB disk space for the AI model
"@
gh release create "v$Version" $zip.FullName --title "Stet v$Version" --notes $notes
