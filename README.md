...
Before sending text to the local LLM, Stet masks URLs (`https://...`), email addresses, file paths, Markdown links, inline code, and any user-defined protected terms, replacing each with an opaque sentinel marker (`__STET_PROTECTED_n__`). The LLM only ever sees natural-language prose. After inference, every masked atom is restored verbatim, and a sentinel-survival check rejects any output that dropped, duplicated, or reordered a marker — so links and variable names can never be corrupted.

### Can I trust the local LLM not to hallucinate or add conversational filler?
**Yes.** All correction modes run with deterministic sampling (temperature 0.0, top_k 1) and a single strict prompt path (`stet/llm/messages.py`) that frames the selected text as content to transform — never as instructions to follow — so a highlighted question is corrected, not answered. If an output is a refusal, conversational filler (*"Sure, here is your corrected text:"*), or diverges beyond the configurable divergence threshold, the edit is automatically rejected.

### Does Stet make reasoning models "think" before correcting?
**No.** Thinking is disabled once at the llama.cpp level for every model (`--reasoning off --reasoning-budget 0`), and Stet sends no per-request reasoning overrides. Corrections begin streaming immediately even on reason-capable models (Qwen3, DeepSeek-R1 distills, Ling, etc.).
